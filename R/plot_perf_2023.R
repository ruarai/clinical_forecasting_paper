...
library(targets)
library(lubridate)
forecasts_data <- tar_read(all_forecasts_data)
occupancy_data <- tar_read(occupancy_data_total)

forecast_quants <- forecasts_data$quants
performance_data <- tar_read(performance_data)

date_range <- c(ymd("2023-01-01"), ymd("2023-06-15"))
...
runs <- forecast_quants %>%
  filter(run_date >= ymd("2023-01-01")) %>% 
  distinct(
    state,
    run_date,
    case_forecast_start
...
performance_data_plot <- performance_data %>%
  filter(ymd(run_date) >= ymd("2023-01-01")) %>%
  mutate(run_date = ymd(run_date)) %>% 
...
cairo_pdf("results/perf2023.pdf",
